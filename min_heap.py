...
class MinHeap:
    ...
    def is_empty(self):
        return False if len(self.heap_array) > 0 else True
    ...
    @staticmethod
    def left_child_index(parent_idx):
        return parent_idx * 2

    @staticmethod
    def right_child_index(parent_idx):
    ...
    def calculate_smaller_child_idx(self, idx):
    ...
    def heapify_down(self):
        ...
        while self.has_child(idx):
            smaller_child_idx = self.calculate_smaller_child_idx(idx)
            parent_value = self.heap_array[idx]
            child_value = self.heap_array[smaller_child_idx]
            if child_value < parent_value:
                self.heap_array[idx], self.heap_array[smaller_child_idx] = child_value, parent_value
            else:
                break
...
if __name__ == "__main__":
    ...
    for _ in range(10):
        min_heap.add(random.randint(1, 101))
